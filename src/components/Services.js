...
import { trackBookingStarted, trackBookingConfirmed, trackWhatsAppClick } from '@/lib/analytics';
...
export default function Services() {
  ...
  const openBooking = (service) => {
    ...
    trackBookingStarted(service.title, service.usd || service.price);
  };
  ...
  const sendBooking = () => {
    ...
    trackBookingConfirmed(s.title, s.usd || s.price);
    trackWhatsAppClick('booking_modal');
    setModal(null);
  };

  const inputClass = "w-full bg-[#1e293b] border border-white/5 rounded-xl px-4 py-2.5 text-white text-sm outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/20 transition-all duration-300 placeholder:text-slate-600";

  return (
    <>
      <section id="services" className="relative py-24 lg:py-28 px-6 section-glow">
        <div className="max-w-6xl mx-auto text-center">
          ...
          >
            Book With <span className="gradient-text">Confidence</span>
          </motion.h2>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            Every driver and guide is personally vetted, reviewed, and insured. Fixed prices. No surprises.
          </motion.p>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 text-left">
            {services.map((s, i) => (
              <motion.div
                ...
                className="group glass rounded-2xl overflow-hidden card-hover gradient-border relative shadow-xl shadow-black/20 flex flex-col"
              >
                <div
                  className="h-44 bg-cover bg-center relative flex items-center justify-center"
                  style={{ backgroundImage: `url('${s.img}')` }}
                >
                  <div className="absolute inset-0 bg-gradient-to-t from-[#111827] via-[#111827]/40 to-transparent" />
                  <span className="text-5xl relative z-10 opacity-90 group-hover:scale-110 transition-transform duration-500">{s.emoji}</span>
                </div>
                <div className="p-6 flex flex-col flex-1">
                  {s.badge && <span className="text-[10px] font-bold uppercase tracking-wider text-amber-400 mb-1">{s.badge}</span>}
                  ...
                  {s.affiliate ? (
                    <a
                      href={s.affiliateUrl} target="_blank" rel="noopener noreferrer"
                      className="block w-full py-3 rounded-full bg-gradient-to-r from-cyan-500 to-cyan-400 text-gray-900 font-semibold text-sm text-center hover:-translate-y-0.5 transition-all duration-300 shadow-md shadow-cyan-500/15"
                    >
                      Get eSIM →
                    </a>
                  ) : (
                    <button
                      ...
                      className="block w-full py-3 rounded-full bg-gradient-to-r from-cyan-500 to-cyan-400 text-gray-900 font-semibold text-sm text-center hover:-translate-y-0.5 transition-all duration-300 shadow-md shadow-cyan-500/15 cursor-pointer"
                    >
        ...
        {modal && (
          ...
          >
            <motion.div
              ...
              className="bg-[#111827] border border-white/10 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto"
            >
              {/* Header */}
              <div className="px-6 pt-6 pb-4 border-b border-white/5">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setModal(null)} className="text-slate-500 hover:text-white text-2xl leading-none transition-colors w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/5">✕</button>
                </div>
              ...
              <div className="px-6 py-5 space-y-4">
                ...
                <div>
                  ...
                  <input type="text" value={form.name} onChange={e => setForm({...form, name: e.target.value})} placeholder="e.g. Sarah Miller" className={inputClass} />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <select value={form.people} onChange={e => setForm({...form, people: e.target.value})} className={inputClass}>
                ...
                <div>
                  ...
                  <input type="text" value={form.hotel} onChange={e => setForm({...form, hotel: e.target.value})} placeholder="e.g. Alila Seminyak / Ngurah Rai Airport" className={inputClass} />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-400 mb-1.5">Notes (optional)</label>
                  <textarea value={form.notes} onChange={e => setForm({...form, notes: e.target.value})} rows={2} placeholder="e.g. 2 large suitcases, arriving at 10pm" className={`${inputClass} resize-none`} />
                </div>
              </div>
              ...
              <div className="px-6 pb-6">
                <button
                  onClick={sendBooking}
                  className="w-full py-3.5 rounded-full bg-[#25D366] text-white font-semibold text-sm flex items-center justify-center gap-2 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-green-900/30 transition-all duration-300 cursor-pointer"
                >
